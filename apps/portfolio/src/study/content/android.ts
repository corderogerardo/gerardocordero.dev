...
export const androidSubject: Subject = {
  ...
  "cards": [
    ...
    },
    {
      ...
      "answer": "Jank is a **dropped frame**: the UI thread didn't finish producing a frame within the display's budget (~16.7ms at 60Hz, ~8.3ms at 120Hz), so the same frame shows twice and the user sees a stutter. The frame pipeline is driven by `Choreographer`, and the work that overruns is almost always on the **main thread**.\n\nThe usual culprits: doing I/O, JSON parsing, bitmap decoding, or heavy computation on the main thread; allocating in a scroll/draw loop (triggering GC); deep view hierarchies causing expensive measure/layout passes; and in Compose, excessive recomposition or unstable parameters.\n\nTo diagnose, measure rather than guess:\n\n- **Android Studio Profiler / system trace (Perfetto)** to see what the main thread is doing during the janky window and which frames missed the deadline.\n- **Jank stats / `FrameMetrics`** and the **Macrobenchmark** library to quantify jank in a repeatable way.\n- **Layout Inspector** and the Compose **recomposition counts** to spot redundant work.\n\nThe fix is almost always the same shape: move work off the main thread (`withContext`), cut allocations, flatten layouts, or make composables skippable.",
      "level": "mid"
    },
    {
    ...
    },
    {
      ...
      "answer": "Every Room `@Database` has a **version number**. When you change the schema (add a column, a table, an index), you bump the version and provide a **`Migration(from, to)`** whose `migrate()` runs the raw `ALTER TABLE`/`CREATE TABLE` SQL to move existing data from the old schema to the new one. Room runs the chain of migrations needed to get a user's on-device database from their version up to the current one — so **existing user data is preserved** across app updates.\n\nIf the schema changes and you **don't** supply a migration, Room throws `IllegalStateException` at open time. The escape hatch is `fallbackToDestructiveMigration()`, which **drops and recreates** the database instead — but that **deletes all local data**. That's fine for a pure cache you can re-fetch, and a data-loss bug for anything the user authored offline.\n\nGuardrails: Room can **export the schema JSON** and provides `MigrationTestHelper` to test that each migration actually upgrades real data correctly. The senior habit is to treat migrations like API changes — version every schema change, write the migration, and test it against the previous schema rather than relying on destructive fallback.",
      "level": "senior"
    },
    {

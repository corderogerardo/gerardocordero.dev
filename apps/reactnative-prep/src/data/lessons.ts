...
export const LESSONS: Lesson[] = [
  // ─────────────────────────── CORE ───────────────────────────
  {
    ...
    html: `<p>Every React Native performance rule — don't block the JS thread, don't do heavy sync work in a render, offload animations to native — flows from one fact: your JavaScript runs on a thread that is separate from the UI thread and can only do one thing at a time. Get that mental model wrong and every profiling session turns into guesswork.</p>
<p>In the <b>old architecture</b> (pre-0.68), JS and native communicated via a JSON bridge — serialising every message to JSON, crossing a thread boundary, and deserialising on the other side. This was the root cause of most RN performance problems: any heavy JS work blocked the bridge and caused dropped frames.</p>
...
<p><b>Red flag:</b> saying JSI makes native calls "free." It removes JSON serialisation, but a call across the boundary still costs a function call and, for non-primitive data, a memory copy — the JS thread is still single-threaded and still your bottleneck.</p>
<p><b>"New Architecture removes the JSON bridge, but the JS thread is still single-threaded — every performance decision I make starts with keeping it unblocked."</b></p>`,
  },
  {
    id: "core-2",
    category: "core",
...
<p><b>Red flag:</b> nesting a <code>&lt;FlatList&gt;</code> inside a <code>&lt;ScrollView&gt;</code> that scrolls the same direction — the inner list either won't scroll or both fight for gesture ownership. Flatten to one scroll container.</p>
<p><b>"Every element I render maps to a real native view, not a DOM node — so I pick the primitive by its native behaviour and constraints, not just its visual result."</b></p>`,
  },
  {
    id: "core-3",
    category: "core",
    ...
    html: `<p><code>StyleSheet.create</code> exists because sending a style object across to native on every single render is wasted work — it registers the object once and hands the native thread a numeric ID instead. Skip it and you pay that cost repeatedly for no visual benefit.</p>
<p>React Native uses <b>Yoga</b> — a cross-platform Flexbox layout engine — instead of the browser's CSS engine. Most flexbox properties work the same, but a few differ: <code>flexDirection</code> defaults to <code>'column'</code> (not <code>row</code>), there's no <code>display: grid</code>, no CSS cascade, and units are density-independent pixels (no <code>px</code>, <code>em</code>, or <code>%</code> except in a few specific props).</p>
...
<p><b>Red flag:</b> treating <code>StyleSheet.create</code> as pure convention and reaching for inline style objects "because it's simpler" — every inline object is a new reference on every render, so it silently reintroduces the cost the API exists to remove.</p>
<p><b>"I always define styles with <code>StyleSheet.create</code>, not inline objects — inline styles create a new reference every render and skip the ID-registration optimisation."</b></p>`,
  },
  {
    id: "core-4",
    category: "core",
    ...
    html: `<p>Unlike a web tab, a backgrounded RN app doesn't pause its JS thread — a forgotten interval, subscription, or animation keeps running (and draining battery) after the user leaves the screen. Cleanup discipline in <code>useEffect</code> isn't boilerplate, it's what prevents that.</p>
<p>In function components with hooks, the lifecycle maps to: mount = first render + effects fire, update = re-render when state/props change + effects fire if deps changed, unmount = cleanup functions in <code>useEffect</code> return values run.</p>
...
<p><b>Red flag:</b> suppressing <code>react-hooks/exhaustive-deps</code> to "fix" an infinite loop instead of fixing the dependency — that lint rule is catching a real stale-closure bug, not being pedantic.</p>
<p><b>"I treat every subscription in <code>useEffect</code> as a leak until it has a matching cleanup — RN doesn't pause the JS thread when the app backgrounds, so an uncleaned listener keeps running."</b></p>`,
  },
  {
    id: "core-5",
    category: "core",
...
<p><b>Red flag:</b> hard-coding pixel cutoffs to detect a notch or dynamic island — device dimensions change every hardware generation. Check capability (safe-area insets, <code>expo-device</code>) instead of guessing a screen size.</p>
<p><b>"I pick the platform-divergence tool by how much the implementations diverge — inline for a single value, <code>Platform.select</code> for a style object, separate <code>.ios</code>/<code>.android</code> files when the logic itself differs."</b></p>`,
  },
  ...
  // ─────────────────────────── EXPO ───────────────────────────
  {
    ...
    html: `<p><b>Managed workflow</b> owns the native project so your team doesn't have to maintain <code>ios/</code>/<code>android/</code> directories or keep up with every native RN upgrade by hand — that's the trade-off, not a beginner-only training-wheels mode. <b>Bare workflow</b> hands you the native projects directly in exchange for full native control.</p>
<p><b>Managed workflow</b> means Expo owns the native project files (<code>ios/</code> and <code>android/</code> directories don't exist in your repo). You configure native behaviour through <code>app.json</code> / <code>app.config.js</code> and Expo config plugins. The build runs on EAS Build (cloud) which generates native projects on the fly. You never manually open Xcode or Android Studio for routine work.</p>
...
<p>The practical guide: <b>start managed</b>. Yes, that means accepting a real cost — you can't hand-edit <code>Info.plist</code> or drop in an arbitrary native SDK without a config plugin. But the alternative, starting bare, means paying the ongoing cost of manually migrating <code>ios/</code>/<code>android/</code> on every RN upgrade — for most apps, that recurring tax is bigger than the plugin ceiling you'd hit. The managed workflow supports 99% of app features through Expo SDK modules, and config plugins let you extend native behaviour without ejecting. Run <code>npx expo prebuild</code> only when you need something a plugin can't express — and even then, commit the generated files so CI can build them. The old concept of "ejecting" is gone; prebuild is the replacement and it's reversible (you can delete and regenerate the native directories). As the app matures and needs a one-off native module, prebuild is the escape hatch, not a reason to have started bare.</p>
...
<p><b>"I default to managed workflow and reach for prebuild only when a specific native requirement outgrows what a config plugin can express — that keeps native-upgrade maintenance off the team until it's actually needed."</b></p>`,
  },
  {
    id: "expo-2",
    category: "expo",
...
<p><b>Red flag:</b> hand-editing a generated file inside <code>ios/</code> or <code>android/</code> instead of writing a plugin — the next <code>prebuild</code> wipes and regenerates those directories, silently discarding the edit.</p>
<p><b>"Config plugins are how I keep native customisation reproducible — the change lives in source control as code, not as a one-off manual edit that prebuild would erase."</b></p>`,
  },
  {
    id: "expo-3",
    category: "expo",
...
<p><b>Red flag:</b> committing a keystore or <code>.p12</code> file "just for this build" — that's a permanent secret in git history. Store credentials on EAS (<code>eas credentials</code>) and give CI only <code>EXPO_TOKEN</code>.</p>
<p><b>"Build profiles let me keep dev, preview, and production as named, reproducible configs instead of one script with branching flags — and credentials never touch the repo."</b></p>`,
  },
  {
    id: "expo-4",
    category: "expo",
...
<p><b>Red flag:</b> treating OTA as a substitute for App Store/Play review when the change touches native code — a new native dependency or an <code>Info.plist</code>/<code>AndroidManifest.xml</code> change requires a new binary and a real submission, no exceptions.</p>
<p><b>"OTA updates let me ship JS fixes without a store review cycle, but the runtime-version check is a safety rail, not red tape — it's what stops a JS bundle from loading into a binary it wasn't built for."</b></p>`,
  },
  {
    id: "expo-5",
    category: "expo",
...
<p><b>"File-based routing means the URL structure and the navigation structure are the same artifact — I don't maintain a separate route config that can drift from the file tree."</b></p>`,
  },
  ...
  // ─────────────────────────── DATA & STATE ───────────────────────────
  {
    ...
    html: `<p><code>useState</code> is a value setter — you hand it the next value. <code>useReducer</code> is a dispatch target for a pure transition function — you hand it an intent (an action), and the reducer decides the next state. Picking the wrong one doesn't break the app, but it makes multi-field state transitions much harder to reason about and test as the component grows.</p>
<p>Both hooks manage state in a function component — the difference is the shape of the update logic. <code>useState</code> is a setter: you call it with the next value (or a function of the old value). <code>useReducer</code> is a dispatch: you send a typed action and a pure function computes the next state.</p>
...
<p><b>"I reach for <code>useReducer</code> the moment state fields start updating together — it turns implicit multi-setter choreography into one pure, testable transition function."</b></p>`,
  },
  {
    id: "data-2",
    category: "data",
    ...
    html: `<p>Zustand lives outside the React tree and uses pub-sub instead of context propagation — a component only re-renders when the specific slice it subscribed to changes, not on every store update. That subscription model is the entire performance argument for choosing it over Context for frequently-changing state.</p>
<p>Zustand stores are created outside React with <code>create()</code>. A store is a function that receives <code>set</code> and returns an object with state and actions. Components subscribe via a selector function — they re-render only when the selected slice changes, not on every store update.</p>
...
<p><b>Red flag:</b> calling the store hook with no selector (<code>useTaskStore()</code>) "for convenience" — it throws away the entire re-render optimisation Zustand is chosen for and re-renders the component on every unrelated store change.</p>
...
<p><b>"I choose selectors and persistence adapters, not whether the store re-renders too much — the subscription model already guarantees that; the discipline is picking the right storage backend for what's sensitive."</b></p>`,
  },
  {
    id: "data-3",
    category: "data",
    ...
    html: `<p>React Query's job is server state — data that is owned by a server and can go stale — while Zustand/useState own UI state that lives only on the device. Conflating the two is why teams end up hand-rolling loading/error/retry/cache-invalidation logic that React Query already solves.</p>
<p>React Query (<code>@tanstack/react-query</code>) manages server state — data that lives on a server and needs to be fetched, cached, and kept fresh. It separates server state from UI state (which Zustand/useState handle) and handles the async lifecycle that you'd otherwise write by hand: loading, error, refetch, retry, pagination, and optimistic updates.</p>
...
<p><b>Red flag:</b> reaching for an optimistic update to hide an initial data load. Optimistic updates work for <b>mutations</b> because you know the intended outcome and can roll it back on error; an initial load has no known outcome to guess — that's a staged/skeleton loading problem, not an optimistic-update problem.</p>
<p><b>"React Query owns server state end to end — cache, staleness, retries, invalidation — so my components only ever describe what data they need, not how to fetch or cache it."</b></p>`,
  },
  {
    id: "data-4",
    category: "data",
    ...
    html: `<p>AsyncStorage and SecureStore aren't interchangeable key-value stores with different names — AsyncStorage is a plaintext file cache for non-sensitive data; SecureStore is a hardware-backed credential vault. Picking AsyncStorage for a token isn't a style choice, it's a data-exposure bug.</p>
<p><code>@react-native-async-storage/async-storage</code> is a simple key-value store backed by plain files on disk: on Android in <code>/data/data/&lt;package&gt;/files/</code>, on iOS in the app's <code>Documents/</code> directory. Both locations are accessible to backups (iTunes/ADB) and — on rooted/jailbroken devices — to any process with elevated permissions. The data is <b>plaintext</b>.</p>
...
<p><b>Red flag:</b> storing an auth token, password, PIN, or any account credential in AsyncStorage "temporarily" — the store is plaintext on disk and readable from backups on rooted/jailbroken devices. Never store in AsyncStorage: auth tokens (access or refresh), passwords, PINs, payment card data, social security numbers, health records, private keys, or any credential that grants access to an account. For these, use <code>expo-secure-store</code>, which writes to the iOS Keychain and Android Keystore — hardware-backed, backup-excluded, and app-sandboxed.</p>
...
<p><b>"I pick storage by sensitivity, not by habit — AsyncStorage for anything a leak only embarrasses, SecureStore for anything a leak compromises."</b></p>`,
  },
  {
    id: "data-5",
    category: "data",
...
<p><b>"I default to last-write-wins with a visible conflict notice, and only reach for CRDTs on the specific data types where concurrent edits are frequent enough that silent loss is unacceptable — full conflict-free merging is a cost I don't pay everywhere by default."</b></p>`,
  },
...
<p><b>Tabs</b> (<code>&lt;Tabs&gt;</code>) render a bottom tab bar (or top on Android, though bottom is the native convention). Each tab maintains its own navigation stack independently — navigating within one tab doesn't affect another. Good for top-level app sections that users switch between frequently.</p>
<p><b>Red flag:</b> nesting a <code>&lt;Tabs&gt;</code> navigator inside a <code>&lt;Stack&gt;</code> at the same navigational level as other stack screens — it breaks the "tabs are always visible at the app's top level" UX expectation users bring from every other app.</p>
...
<p><b>"I pick the navigation container by the UX contract it makes with the user — Stack for drill-down, Tabs for frequently-switched top-level sections, Modal for a contextual action that returns you to where you were, Drawer only when the information architecture genuinely can't fit a tab bar."</b></p>`,
  },
  {
    id: "nav-2",
    category: "navigation",
...
<p>Nested layouts compose: a root <code>_layout.tsx</code> with tabs, each tab's directory with its own Stack layout. The stack header renders inside the tab bar content area — it doesn't fight the tab bar for space. For maximum control, use <code>Stack.Screen</code> inside a screen file to set per-screen options (title, header right button, presentation style) without touching the layout file.</p>`,
  },
  {
    ...
    html: `<p><b>Custom URL schemes</b> are a convenience mechanism — any app can register the same scheme, so they carry no ownership guarantee. <b>Universal/App Links</b> are a security mechanism — cryptographically bound to a domain only you control. Confusing the two is how sensitive tokens end up in an interceptable URL.</p>
<p>Deep links open your app to a specific screen from outside — another app, a browser, an email. Two mechanisms: <b>custom URL schemes</b> (<code>myapp://profile/123</code>) and <b>Universal Links</b> (iOS) / <b>App Links</b> (Android) — regular HTTPS URLs (<code>https://myapp.com/profile/123</code>) that the OS routes to your app instead of the browser.</p>
<p>Custom schemes are easy to implement but have a security problem: any other app can register the same scheme and intercept your deep links. Universal Links are cryptographically bound to your domain via an HTTPS-served JSON file (<code>apple-app-site-association</code> for iOS, <code>assetlinks.json</code> for Android) that only you control — they're the right choice for any sensitive flow.</p>
<p><b>Red flag:</b> putting a password-reset link or OAuth code in a custom-scheme URL (<code>myapp://reset?token=…</code>) — any app can register that scheme and capture the token. Sensitive flows require Universal/App Links, which are domain-verified.</p>
...
<p><b>"Custom schemes are for convenience deep-linking; Universal Links are for anything security-sensitive — the domain-verification file is what makes them unspoofable."</b></p>`,
  },
  {
    id: "nav-4",
    category: "navigation",
...
<p><b>Red flag:</b> reaching for <code>useGlobalSearchParams</code> to pass data between sibling screens — that's route params doing the job of shared state. A Zustand store or context is the correct tool.</p>
<p><b>"I pass an ID through route params and fetch the data from a shared store on arrival — encoding payloads in the URL breaks it as a bookmark and doesn't survive a deep link."</b></p>`,
  },
  {
    id: "nav-5",
    category: "navigation",
    ...
    html: `<p>Auth flows in expo-router have one key challenge: the app renders before it knows whether the user is authenticated (async session restore). The naive approach — conditionally rendering a login screen — produces a flash of the protected content before the redirect fires. The fix is a fixed sequence, not a collection of tips:</p>
<p>1. <b>Hold rendering.</b> Start a <code>loading</code> flag at <code>true</code> and return <code>null</code> or the splash screen while it's true — never render protected routes speculatively.<br>2. <b>Restore the session.</b> Attempt to read the stored credential (<code>SecureStore.getItemAsync('refresh_token')</code>) and refresh it.<br>3. <b>Resolve auth state.</b> Set <code>loading = false</code> with a definite authenticated/unauthenticated result — no in-between state.<br>4. <b>Redirect declaratively.</b> Use route-group <code>redirect</code> props or <code>&lt;Redirect&gt;</code>, which fire synchronously in render, instead of an imperative <code>router.replace</code> in an effect that runs a frame late.</p>
...
<p><b>Red flag:</b> conditionally rendering <code>&lt;LoginScreen /&gt;</code> vs the app inside a single component based on a flag that starts <code>false</code> — that renders protected content for one frame before the auth check resolves. Hold rendering with a loading gate instead.</p>
<p><b>"Auth guards should hold rendering until session state is definite, then redirect declaratively — an imperative redirect after the fact is what causes the flash."</b></p>`,
  },
...
<p><b>Red flag:</b> wrapping every value and function in <code>useMemo</code>/<code>useCallback</code> "for performance" without profiling first — each call adds bookkeeping cost, so blanket memoisation can make a component slower, not faster.</p>
<p><b>"I memoise only after profiling shows a specific value or callback is causing a specific, unnecessary re-render — not as a default habit."</b></p>`,
  },
  {
    id: "perf-2",
    category: "performance",
...
<p><b>Red flag:</b> using the array index as <code>keyExtractor</code> — when items are inserted, removed, or reordered, React matches the wrong item to the wrong key, causing stale row content and broken animations. Use a stable ID from the data.</p>
...
<p><b>"FlatList unmounts off-screen rows; FlashList recycles the native views instead of destroying them — that's the whole reason it wins on fast-scrolling, high-volume feeds and not on a 20-item list."</b></p>`,
  },
  {
    id: "perf-3",
    category: "performance",
...
<p><b>Red flag:</b> rendering a remote image with no explicit size "because it'll size itself" — it won't paint until native measures it, producing a visible layout shift as content jumps once the image loads.</p>
<p><b>"I never ship an unsized remote image — either I know the dimensions up front or I reserve the space with an aspect-ratio container, so nothing shifts once the image loads."</b></p>`,
  },
  {
    id: "perf-4",
    category: "performance",
...
<p>The universal diagnosis flow, in order — don't skip a step: 1. <b>Isolate the thread.</b> Is the JS thread pegged, or is it a native draw problem? 2. <b>Prove it with the matching tool.</b> React DevTools Profiler for re-renders, Flipper's CPU profiler for JS-thread work, Systrace/Instruments for native frame timing. 3. <b>Fix the outermost layer first</b> — a re-render fix is wasted effort if the actual bottleneck is native layout inflation.</p>`,
  },
  {
    id: "perf-5",
    category: "performance",
...
<p><b>Red flag:</b> blaming "React Native performance" for janky gesture-driven animation built with the basic <code>Animated</code> API — the fix is moving the animation logic onto the UI thread with Reanimated, not a native rewrite.</p>
<p><b>"I pick by where the animation logic needs to run — Animated for simple transitions, Reanimated when gesture response has to stay on the UI thread, Skia when I'm drawing pixels the native view system can't express."</b></p>`,
  },
];

...
export const PROMPTS: Prompt[] = [
  {
    ...
    "reveal": [
      ...
      {
        "label": "Solution",
        "html": "<div class=\"code\">import { useEffect, useState } from &quot;react&quot;;\n\nexport function useDebounce&lt;T&gt;(value: T, delay = 300): T {\n  const [debounced, setDebounced] = useState&lt;T&gt;(value);\n\n  useEffect(() =&gt; {\n    const id = setTimeout(() =&gt; setDebounced(value), delay);\n    return () =&gt; clearTimeout(id);\n  }, [value, delay]);\n\n  return debounced;\n}\n\n// Usage:\n// const debouncedQuery = useDebounce(query, 400);\n// useEffect(() =&gt; { search(debouncedQuery); }, [debouncedQuery]);</div><p>Each render with a new <code>value</code> schedules a fresh timer and the cleanup cancels the previous one, so the value only &quot;lands&quot; once typing pauses. This is the trailing-edge debounce most search inputs want. <b>I always clear the previous timer in the effect cleanup — that single line is what turns a plain setTimeout into a real debounce.</b></p>"
      }
    ]
  },
  {
    ...
    "reveal": [
      ...
      {
        "label": "Solution",
        "html": "<div class=\"code\">import { useEffect, useRef } from &quot;react&quot;;\n\nexport function useInterval(callback: () =&gt; void, delay: number | null): void {\n  const savedCallback = useRef(callback);\n\n  // Always keep the ref pointing at the freshest callback.\n  useEffect(() =&gt; {\n    savedCallback.current = callback;\n  }, [callback]);\n\n  useEffect(() =&gt; {\n    if (delay === null) return; // paused\n    const id = setInterval(() =&gt; savedCallback.current(), delay);\n    return () =&gt; clearInterval(id);\n  }, [delay]);\n}</div><p>The ref decouples &quot;which function runs&quot; from &quot;when the interval restarts,&quot; so the timer survives re-renders yet never fires a stale closure. This is the canonical Dan Abramov pattern. <b>I keep the callback in a ref so I can restart the interval only when the delay changes, never when the callback identity does.</b></p>"
      }
    ]
  },
  {
    ...
    "reveal": [
      ...
      {
        "label": "Solution",
        "html": "<div class=\"code\">import { useEffect, useRef } from &quot;react&quot;;\n\nexport function usePrevious&lt;T&gt;(value: T): T | undefined {\n  const ref = useRef&lt;T | undefined&gt;(undefined);\n\n  useEffect(() =&gt; {\n    ref.current = value;\n  }, [value]);\n\n  return ref.current;\n}\n\n// const prevCount = usePrevious(count);\n// if (prevCount !== undefined &amp;&amp; count &gt; prevCount) animateUp();</div><p>Because the effect runs <i>after</i> render commits, the component renders with last cycle's value still in the ref, then the ref is advanced — giving you a clean &quot;before/after&quot; pair. Note: with React 19, <code>useRef</code> requires an explicit initial argument, hence <code>useRef&lt;T | undefined&gt;(undefined)</code>. <b>I read the ref before updating it in the effect, so the previous render's committed value is always what comes back.</b></p>"
      }
    ]
  },
  {
    ...
    "reveal": [
      ...
      {
        "label": "Solution",
        "html": "<div class=\"code\">export function groupBy&lt;T, K extends PropertyKey&gt;(\n  items: readonly T[],\n  getKey: (item: T) =&gt; K,\n): Record&lt;K, T[]&gt; {\n  return items.reduce((acc, item) =&gt; {\n    const key = getKey(item);\n    (acc[key] ??= []).push(item);\n    return acc;\n  }, {} as Record&lt;K, T[]&gt;);\n}\n\n// const byRole = groupBy(users, (u) =&gt; u.role);\n// byRole.admin // User[]</div><p>The <code>K extends PropertyKey</code> bound is what makes the index signature legal, and <code>??=</code> keeps bucket creation a one-liner. Callers get precise return types inferred from <code>getKey</code>. <b>Constraining K to PropertyKey is what lets TypeScript treat the accumulator as an indexable Record instead of falling back to any.</b></p>"
      }
    ]
  },
  {
    ...
    "reveal": [
      {
        "label": "Approach",
        "html": "<ol><li><b>Isolate:</b> <code>React.memo</code> only does a shallow prop compare, so the re-render has to come from a prop reference changing on every parent render — check <code>style</code> and <code>onPress</code> first, they're the usual suspects.</li><li><b>Prove it:</b> the inline <code>style={{ padding: 8 }}</code> object and the inline <code>onPress</code> arrow are <b>new references every render</b>, so the shallow compare always fails even though the values look identical.</li><li><b>Fix it:</b> hoist the static style out of render with <code>StyleSheet.create</code>, and give each row a stable <code>onPress</code> by memoizing the handler with <code>useCallback</code> and letting the child bind its own id, so the callback identity never changes per render.</li></ol><p><b>Red flag:</b> reaching for <code>React.memo</code> as a reflex without checking why the shallow compare is failing — wrapping the component here without hoisting the style and handler still re-renders every row on every parent update.</p>"
      },
      {
        "label": "Solution",
        "html": "<div class=\"code\">const styles = StyleSheet.create({ row: { padding: 8 } });\n\nconst Row = React.memo(function Row(\n  { item, onPress }: { item: Item; onPress: (id: string) =&gt; void },\n) {\n  return (\n    &lt;Pressable style={styles.row} onPress={() =&gt; onPress(item.id)}&gt;\n      &lt;Text&gt;{item.name}&lt;/Text&gt;\n    &lt;/Pressable&gt;\n  );\n});\n\nfunction List({ items }: { items: Item[] }) {\n  // stable identity across renders\n  const handlePress = useCallback((id: string) =&gt; select(id), []);\n  return (\n    &lt;&gt;{items.map((item) =&gt; (\n      &lt;Row key={item.id} item={item} onPress={handlePress} /&gt;\n    ))}&lt;/&gt;\n  );\n}</div><p>Now <code>style</code> is a stable <code>StyleSheet</code> reference and <code>onPress</code> is a single memoized function, so <code>memo</code>'s shallow compare passes and only rows whose <code>item</code> actually changed re-render. The child closes over <code>item.id</code> itself, keeping the parent's callback identity constant. <b>React.memo is only as good as the stability of the props you pass it — fix the reference, not just add the wrapper.</b></p>"
      }
    ]
  },
  {
    ...
    "reveal": [
      ...
      {
        "label": "Solution",
        "html": "<div class=\"code\">export class LRUCache&lt;K, V&gt; {\n  private map = new Map&lt;K, V&gt;();\n  constructor(private capacity: number) {}\n\n  get(key: K): V | undefined {\n    if (!this.map.has(key)) return undefined;\n    const value = this.map.get(key)!;\n    this.map.delete(key);     // re-insert to mark as most-recently used\n    this.map.set(key, value);\n    return value;\n  }\n\n  set(key: K, value: V): void {\n    if (this.map.has(key)) this.map.delete(key);\n    this.map.set(key, value);\n    if (this.map.size &gt; this.capacity) {\n      const oldest = this.map.keys().next().value as K;\n      this.map.delete(oldest);\n    }\n  }\n}</div><p>Exploiting <code>Map</code>'s insertion-order guarantee gives a clean O(1) LRU; the delete-then-set dance is what keeps the ordering honest on every touch. <b>A plain Map's insertion order gives me LRU semantics for free — no need to hand-roll a doubly linked list.</b></p>"
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "reveal": [
      ...
      {
        "label": "Solution",
        "html": "<div class=\"code\">export function normalizeById&lt;T&gt;(\n  items: readonly T[],\n  getId: (item: T) =&gt; string,\n): { byId: Record&lt;string, T&gt;; ids: string[] } {\n  const byId: Record&lt;string, T&gt; = {};\n  const ids: string[] = [];\n  for (const item of items) {\n    const id = getId(item);\n    if (!(id in byId)) ids.push(id);\n    byId[id] = item;\n  }\n  return { byId, ids };\n}\n\n// const { byId, ids } = normalizeById(messages, (m) =&gt; m.id);\n// byId[selectedId] // O(1) lookup; ids preserves render order</div><p>Separating <code>byId</code> from <code>ids</code> gives O(1) entity access while keeping a stable render order — exactly the normalized cache shape that makes version-based invalidation and optimistic updates cheap. <b>Normalizing into byId + ids up front is what makes O(1) lookups and targeted re-renders possible later — retrofitting it after the UI is built costs much more.</b></p>"
      }
    ]
  },
  {
    ...
    "reveal": [
      ...
      {
        "label": "Solution",
        "html": "<div class=\"code\">import { useCallback, useMemo, useState } from &quot;react&quot;;\n\ntype ToggleHandlers = {\n  toggle: () =&gt; void;\n  setOn: () =&gt; void;\n  setOff: () =&gt; void;\n};\n\nexport function useToggle(\n  initial = false,\n): [boolean, ToggleHandlers] {\n  const [value, setValue] = useState(initial);\n\n  const toggle = useCallback(() =&gt; setValue((v) =&gt; !v), []);\n  const setOn = useCallback(() =&gt; setValue(true), []);\n  const setOff = useCallback(() =&gt; setValue(false), []);\n\n  const handlers = useMemo(\n    () =&gt; ({ toggle, setOn, setOff }),\n    [toggle, setOn, setOff],\n  );\n\n  return [value, handlers];\n}\n\n// const [isOpen, { toggle, setOff }] = useToggle();</div><p>Functional updates make the callbacks dependency-free, so <code>useCallback([])</code> yields permanently stable references; the memoized <code>handlers</code> object means passing it to a memoized child won't trigger needless re-renders. <b>The functional updater is what lets toggle stay dependency-free and still be correct if it's called twice in the same tick.</b></p>"
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "reveal": [
      ...
      {
        "label": "Senior signals",
        "html": "<ul><li>Reaches for a <b>resumable/chunked protocol</b> and an <b>idempotency key</b> rather than a naive single POST with retry. <b>Red flag:</b> retrying a whole-file POST on failure — on flaky cellular that re-uploads all 40MB from zero every time it drops, instead of resuming from the last acked part.</li><li>Knows large files must be <b>streamed from disk in native code</b>, not held in the JS heap, and uses OS background transfer APIs for real background completion.</li><li><b>Persists upload state</b> so a force-kill resumes instead of restarting; resumes by asking the server which parts exist.</li><li>Backoff with <b>jitter</b>, transient-vs-permanent error classification, and token refresh before retry.</li><li>Verifies <b>integrity per part</b> (checksums/ETags) and prevents duplicate objects.</li><li>Observability: upload success rate, p95 time-to-complete, resume rate, bytes wasted on restart, per-part failure reasons.</li><li><b>Chunked upload with persisted per-part state is more moving parts than a single POST with retry — but that's exactly what turns a dropped connection at 70% into a 30% resume instead of a from-scratch re-upload, and the same state machine covers a small photo or a multi-gigabyte file without changes.</b></li></ul>"
      }
    ]
  },
  {

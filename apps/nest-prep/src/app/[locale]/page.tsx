...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const STATS = [
    { value: "140+", label: es ? "Tarjetas de P&R" : "Q&A flashcards" },
    { value: "31", label: es ? "Ejercicios de práctica" : "Practice prompts" },
    { value: "36", label: es ? "Temas de la guía" : "Study-guide topics" },
    {
    ...
    <div className="animate-fade-up space-y-12">
      <section className="space-y-5">
        <span className="inline-flex items-center gap-2 rounded-full border border-border bg-surface px-3 py-1 text-xs font-medium text-muted">
          ...
          {es
            ? "Senior NestJS · Node.js · listo para la entrevista"
        ...
        <p className="max-w-2xl text-lg text-muted">
          {es ? (
            <>
              ...
              pide un puesto senior de NestJS / Node.js — la inyección de
              dependencias, el ciclo de vida de la petición, los microservicios,
              el event loop y la arquitectura detrás de todo.
            </>
          ) : (
            <>
              Drill technical Q&amp;A as flashcards, test yourself with a quiz,
              rehearse spoken answers, and master every requirement a senior
              NestJS / Node.js role asks for — dependency injection, the request
              lifecycle, microservices, the event loop, and the architecture
              behind it all.
            </>
          )}
        </p>
...
const METHOD_ES = [
  ...
  },
  {
    icon: "🔀",
    title: "Intercalado",
    body: "Mezcla categorías y niveles en vez de estudiar un solo tema. Filtra por Microservicios, Seguridad, Arquitecto… y mezcla para combatir la falsa fluidez.",
  },
  {
    icon: "🛠",

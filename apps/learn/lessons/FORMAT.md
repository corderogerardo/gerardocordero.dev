...
3. **`md` / `prompt` / `intro` fields are ARRAYS of double-quoted strings**, one block each.
   ...
   A block that starts and ends with ``` renders as a plain (unhighlighted) code
   block — fine for tiny inline snippets; use a `code` step for anything substantial.
...
The Python course lives in `../lessons-python/` — see `../lessons-python/FORMAT-PYTHON.md` for the Python-specific rules on top of this schema.

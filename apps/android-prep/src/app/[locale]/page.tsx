...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const STATS = [
    { value: "200+", label: es ? "Tarjetas de P&R" : "Q&A flashcards" },
    { value: "17", label: es ? "Ejercicios de práctica" : "Practice prompts" },
    { value: "34", label: es ? "Temas de la guía" : "Study-guide topics" },
    {
    ...
    <div className="animate-fade-up space-y-12">
      <section className="space-y-5">
        <span className="inline-flex items-center gap-2 rounded-full border border-border bg-surface px-3 py-1 text-xs font-medium text-muted">
          ...
          {es
            ? "Senior Android · Kotlin & Compose · listo para la entrevista"
            : "Senior Android · Kotlin & Compose · interview ready"}
        </span>
        <h1 className="text-4xl font-extrabold leading-[1.1] sm:text-5xl">
          ...
          <span className="bg-gradient-to-r from-accent to-accent-2 bg-clip-text text-transparent">
            {es ? "con tu historia ya dominada." : "already knowing your story."}
          </span>
        </h1>
        <p className="max-w-2xl text-lg text-muted">
          {es
            ? "Ensaya tus pitches, practica preguntas de Kotlin y Jetpack Compose con tarjetas, ponte a prueba con un quiz y domina cada requisito que pide un puesto senior o staff de Android — coroutines, Flow, Compose, arquitectura, rendimiento y testing."
            : "Rehearse your pitches, drill Kotlin and Jetpack Compose Q&A as flashcards, test yourself with a quiz, and master every requirement a senior or staff Android role asks for — coroutines, Flow, Compose, architecture, performance, and testing."}
        </p>
...
const METHOD_ES = [
  ...
  },
  {
    icon: "🔀",
    title: "Intercalado",
    body: "Mezcla categorías y niveles en vez de estudiar un solo tema. Filtra por Rendimiento, Seguridad, Arquitecto… y mezcla para combatir la falsa fluidez.",
  },
  {
    icon: "🛠",

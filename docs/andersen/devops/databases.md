...
CP vs AP isn't a fixed label per engine — it's a property of how you deploy and configure it. A single-primary Postgres/MySQL setup with synchronous replicas is CP-leaning by default, but a cluster with async read replicas is already serving possibly-stale reads, which is AP behavior with a relational engine. Cassandra and DynamoDB expose *tunable* consistency (per-query read/write quorum settings) — you can run either one CP-leaning (`QUORUM`/`QUORUM` reads and writes) or AP-leaning (`ONE`) depending on the call. The schema-rigidity/join/ACID trade-off (fixed schema, joins, transactions vs. horizontal scale and flexible document shape) is real, but it's a separate axis from CAP — don't conflate "relational" with "CP" or "NoSQL" with "AP"; both are choices made at the deployment and query level, not inherent to the category.
...
Replication is the mechanism: a primary takes writes and streams its write-ahead log to one or more replicas, which serve reads and stand ready to be promoted if the primary dies. The design axis that actually matters is **synchronous vs asynchronous** replication — synchronous means a write isn't acked to the client until a replica confirms it too (zero data loss on failover *for that configured quorum, under the failure model you designed for* — higher write latency); asynchronous acks immediately and ships the log after (faster writes, but a crash right after an ack can lose the last few transactions that never made it to a replica).

The failure mode to design against is the same split-brain problem as any HA cluster: if a network partition isolates the primary from its replicas, and the replicas independently promote one of themselves, you now have two primaries diverging — writes on each side that the other doesn't know about, and no automatic way to merge them without picking a loser. The fix is quorum-gated promotion, but the coordinator differs by stack: Postgres commonly relies on an external tool like Patroni backed by etcd/Consul for consensus, while MySQL Group Replication or Galera build the consensus/quorum logic into the engine itself. Don't assume every cluster uses the same coordinator — the point that transfers is "promotion requires a quorum," not a specific tool.

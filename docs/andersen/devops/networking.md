...
The certificate chain is what makes the identity claim trustworthy: your browser doesn't trust `example.com`'s cert directly — it verifies a signature chain up to a root CA it already trusts, through zero or more intermediates. Keep two *separate* checks distinct here. **Chain validation** walks those signatures to a trusted root; it fails on an expired/untrusted intermediate, or a self-signed leaf that isn't itself installed as a trusted root. **Hostname verification** is a different check: a perfectly valid, fully-trusted chain still fails if the cert's subject/SAN doesn't match the host you connected to. That's why "wrong hostname" and "untrusted chain" are distinct errors — one means the identity didn't match, the other means the trust path didn't hold.
...
The browser's same-origin policy already blocks a script on `evil.com` from reading responses from `bank.com` — CORS is the *relaxation* mechanism, not the restriction. A server opts in to being read cross-origin by sending `Access-Control-Allow-Origin`. For any **non-safelisted** request the browser first sends a preflight `OPTIONS` to check the server allows the method and headers. A *simple* request — `GET`, `HEAD`, or `POST` with only safelisted headers and content types — skips preflight, but a non-safelisted method (`PUT`, `DELETE`, …) *or* a non-safelisted header (`Authorization`, a custom `X-…`) triggers it, even on a `GET`. Note `Authorization` is just a request header that trips preflight; that's separate from Fetch's `credentials: 'include'` mode, which is what actually attaches cookies.

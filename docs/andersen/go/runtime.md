...
**Red flag:** Using `t.Fatal`/`FailNow` inside a goroutine launched from within a test — `Fatal` calls `runtime.Goexit`, which only unwinds the goroutine that called it; the failure itself is still recorded (the test won't report a pass), but the test function keeps running past that point in every other goroutine, including the main test goroutine, so cleanup and later assertions can execute against a half-finished state before the test ultimately fails. The fix is to have the child goroutine send its error back over a channel and call `t.Fatal` from the test's own goroutine if it needs to stop the test immediately.

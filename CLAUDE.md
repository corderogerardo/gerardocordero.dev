...
| `apps/learn` | PawWalk Academy: interactive courses (iOS/Swift, Android/Kotlin, Ruby/Rails, Python/FastAPI+AI, Go/net-http+backend) sharing one no-build engine; validate with `cd apps/learn && node tools/validate.mjs` (no arg validates all five lesson dirs) or `cd apps/learn && node tools/validate.mjs [dir]` (dir: lessons \| lessons-android \| lessons-ruby \| lessons-python \| lessons-go). Deployed static → Cloudflare Pages (`academy.gerardocordero.dev`) via path-filtered `deploy-learn` in `ci.yml` — no build step, deploys the dir as-is (Pages auto-ignores dotfiles/`node_modules`). | ❌ static site, no build — validator is its gate |

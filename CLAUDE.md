...
| `apps/nest-prep` | Static Next.js 16 export → Cloudflare Pages (`nestjs.gerardocordero.dev`). NestJS / Node-backend interview-prep study site. Built on `@gerardocordero/prep-kit` (clone of `apps/template-prep`). | ✅ typecheck + lint (deploy: path-filtered `deploy-nest-prep` in `ci.yml`) |
| `apps/nestjs-prep/Nestjs` | Static Next.js 16 export → Cloudflare Pages (`nestjs.gerardocordero.dev`). NestJS + Node.js interview-prep study site. Registered in `pnpm-workspace.yaml` by its nested path. | ✅ typecheck + lint (deploy: not yet wired — add `deploy-nestjs-prep` to `ci.yml` once the CF project exists) |

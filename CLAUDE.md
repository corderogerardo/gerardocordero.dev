...
| `apps/learn` | PawWalk Academy: interactive courses (iOS/Swift, Android/Kotlin, Ruby/Rails) sharing one no-build engine; validate with `node tools/validate.mjs [lessons-android\|lessons-ruby]`. | ❌ static site, no build — validator is its gate |

...
| `apps/learn` | PawWalk Academy: interactive courses (iOS/Swift, Android/Kotlin, Ruby/Rails) sharing one no-build engine; validate with `node tools/validate.mjs [lessons-android\|lessons-ruby]`. Deployed static → Cloudflare Pages (`academy.gerardocordero.dev`) via path-filtered `deploy-learn` in `ci.yml` — no build step, deploys the dir as-is (Pages auto-ignores dotfiles/`node_modules`). | ❌ static site, no build — validator is its gate |

...
| `apps/learn` | PawWalk Academy: interactive courses (iOS/Swift, Android/Kotlin, Ruby/Rails, Python/FastAPI+AI, Go/net-http+backend) sharing one no-build engine (`index.html` + `lessons*/*.js` + `app.js`). **That static engine is what `academy.gerardocordero.dev` serves** — the Next.js app under `src/` builds but has never been deployed. Gates: `node tools/validate.mjs` (no arg validates all lesson dirs, or pass one: lessons \| lessons-android \| lessons-ruby \| lessons-python \| lessons-go \| lessons-node), `node tools/i18n-check.mjs` (translation-overlay drift), `node --test` (40 tests) — all three run in path-filtered `validate-learn` in `ci.yml`. | ❌ not in pnpm loop — validator + i18n-check + `node --test` are its gates |
| `apps/learn-reactnative` | Next.js 16 static export (Tailwind + shadcn) mounted at `academy.gerardocordero.dev/reactnative/` via `basePath`. Senior React Native interview practice: 150 coding challenges with an in-browser JS test runner, 6 live game previews, and a 91-card flashcard deck generated at build time from `docs/react-native-senior/*.md`. `deploy-learn` builds it and copies `out/` into the academy upload as `reactnative/`. | ✅ typecheck + lint |
